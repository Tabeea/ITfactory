...
def write(file_name, data):
    with open(file_name, "w") as f: # "w" = mod deschidere fisier (write - scriere)
        f.writelines(data) # modul write suprascrie intregul fisier (sterge tot si scrie de la zero)

write("data2.txt", ['1', '7', '3', '3', 'abc'])

def append(file_name, data):
    with open(file_name, "a") as f: # "a" = mod deschidere fisier (append - adaugare)
        f.writelines(data)  # modul append nu sterge si v-a scrie in continuarea ultimului caracter din fisier
